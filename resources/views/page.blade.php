...
@section('styles')
    <style>
        button.favorite-btn {
            ...
            padding: 0;
            /* Remove padding to eliminate extra spacing */
            cursor: pointer;
        }
                                        ...
                                        @if (auth()->check())
                                            @if ($item->favorites->contains('user_id', auth()->id()))
                                                <form action="{{ route('post.unfavourite', $item->id) }}" method="POST">
                                                    ...
                                                    @method('DELETE')
                                                    <button
                                                        class="favorite-btn {{ auth()->check() &&auth()->user()->favorites($item->id)? 'favorited': '' }}"
                                                        data-blog-id="{{ $item->id }}">
                                                        <i style="color: red"
                                                            class="fa-heart {{ auth()->check() &&auth()->user()->favorites($item->id)? 'fas': 'far' }}"></i>
                                                    </button>
                                                </form>
                                            @else
                                                <form action="{{ route('post.favourite', $item->id) }}" method="POST">
